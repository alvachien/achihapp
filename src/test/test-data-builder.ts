...
import { AppLanguage, AppLanguageJson, HomeDef, HomeMember, HomeMemberRelationEnum, UserAuthInfo,
  DocumentType, TranType, Account, AccountCategory, AccountCategoryJson, AssetCategory, AssetCategoryJson, ControlCenter, 
  Currency, CurrencyJson, DocumentTypeJson, TranTypeJson, AccountJson, AccountExtraAdvancePayment, ControlCenterJson,
  Order, OrderJson, Plan, AccountExtraLoan, TemplateDocADP, AccountStatusEnum, financeAccountCategoryBorrowFrom,
...
export class TestDataBuilder {
  ...
  public buildFinAccounts(): void {
    ...
    brwInfo.startDate = startOfDay(addMonths(new Date(), -1));
    ...
    lendto.startDate = startOfDay(addMonths(new Date(), -1));
    lendto.endDate = addYears(brwInfo.startDate, 1);
    for (let i = 0; i < 5; i++) {
      const tmpdoc: TemplateDocLoan = new TemplateDocLoan();
      tmpdoc.DocId = i + 1;
      tmpdoc.TranAmount = 8333.34;
  ...
  public buildFinPlans(): void {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      obj.TargetDate = addYears(obj.StartDate, 1);
