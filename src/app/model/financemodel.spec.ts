...
import { addYears, addMonths, startOfDay, isBefore, isEqual, isAfter, isWithinInterval, addDays } from 'date-fns';
...
describe('AccountExtraAdvancePayment', () => {
  ...
  it('#5. date range is a must', () => {
    ...
    instance.RepeatType = hih.RepeatFrequencyEnum.Month;
    instance.Comment = 'test';
    instance.StartDate = addDays(instance.EndDate, 1);
    expect(instance.isValid).toEqual(false);
  });
  ...
  it('#8. clone shall work', () => {
    ...
    expect(isEqual(instance.StartDate, instance2.StartDate)).toBeTruthy();
    expect(isEqual(instance.EndDate, instance2.EndDate)).toBeTruthy();
  });
  ...
  it('#9. onSetData and writeObject shall work', () => {
    ...
    expect(instance2.Comment).toEqual(instance.Comment);
    expect(instance2.RepeatType).toEqual(instance.RepeatType);
    expect(isEqual(startOfDay(instance.StartDate), startOfDay(instance2.StartDate))).toBeTruthy();
    expect(isEqual(startOfDay(instance.EndDate), startOfDay(instance2.EndDate))).toBeTruthy();
  });
});
...
describe('AccountExtraLoan', () => {
  ...
  xit('#5. isAccountValid', () => {
    ...
    instance.endDate = addDays(instance.startDate, -1);
    expect(instance.isAccountValid).toBeFalsy();
    instance.endDate = addYears(instance.startDate, 1);
    expect(instance.isAccountValid).toBeFalsy();
...
describe('Order', () => {
  let instance: Order;
  let fakeData: TestDataBuilder;

  beforeAll(() => {
    fakeData = new TestDataBuilder();
  ...
  it('#7. onVerify: valid from must earlier than valid to', () => {
    instance.Name = 'test';
    instance.ValidTo = addMonths(instance.ValidFrom!, -1);
    instance.SRules.push(new SettlementRule());
