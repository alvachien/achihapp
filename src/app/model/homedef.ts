...
export class HomeMsg {
  ...
  get SendDateFormatString(): string {
    return format(this._senddate, hih.DateDisplayFormat);
  }
  get Title(): string {
    return this._title;
  }
  ...
  public onSetData(data: any): void {
    ...
    if (data && data.sendDate) {
      this._senddate = parse(data.sendDate, hih.DateDisplayFormat, new Date());
    }
    if (data && data.title) {
  ...
  public writeJSONObject(): IHomeMsgJson {
    const hmj: IHomeMsgJson = {
      ...
      userTo: this._usrto,
      sendDate: format(this._senddate, hih.DateDisplayFormat),
      title: this._title,
